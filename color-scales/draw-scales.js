...
d3ColorScales.map((type) => {
  const container = contents.append('div').attr('class', 'scale-type');

  container.append('h3').text(type.title);

  type.scales.map((scaleName) => {
    container.append('div').text(scaleName);
...
const customScalesContainer = contents.append('div').attr('class', 'scale-type');

customScalesContainer.append('h3').text('Custom');

// Helper function to add a custom scale to our chart
const addCustomScale = (name, scale) => {
  customScalesContainer.append('div').text(name);
...
addCustomScale('interpolate-rgb', d3.interpolateRgb('cyan', 'tomato'));

addCustomScale('interpolate-hsl', d3.interpolateHsl('cyan', 'tomato'));

addCustomScale('interpolate-hcl', d3.interpolateHcl('cyan', 'tomato'));

addCustomScale(
  'interpolate-hcl-steps',
  interpolateWithSteps(6).map(
    // Generates five colors
